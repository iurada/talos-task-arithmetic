...
from lora import hook_lora_pretrained_vit, LinLoRAImageEncoder
...
def finetune(rank, args):
    ...
    ft_path = os.path.join(args.save, train_dataset, "linlora_finetuned.pt")
    zs_path = os.path.join(args.save, train_dataset, "linlora_zeroshot.pt")
    ...
    if args.load is not None and args.load.endswith("pt"):
        image_encoder = LinLoRAImageEncoder.load(args.load)

    else:
        print("Building image encoder.")
        image_encoder = LinLoRAImageEncoder(args)
    ...
    if args.save is not None and is_main_process():
        ...
        model_path = os.path.join(ckpdir, "linlora_zeroshot.pt")
    ...
    if args.save is not None and is_main_process():
        zs_path = os.path.join(ckpdir, "linlora_zeroshot.pt")
        ft_path = os.path.join(ckpdir, "linlora_finetuned.pt")
