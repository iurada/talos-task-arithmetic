...
def finetune(rank, args):
    ...
    linearized_finetuning = args.finetuning_mode in ["linear", 'linear_cossim']
    ...
    ft_path = (
        os.path.join(args.save, train_dataset, f"{args.finetuning_mode}_finetuned.pt")
        if linearized_finetuning
        else os.path.join(args.save, train_dataset, "finetuned.pt")
    )
    zs_path = (
        os.path.join(args.save, train_dataset, f"{args.finetuning_mode}_zeroshot.pt")
        if linearized_finetuning
    ...
    if args.save is not None and is_main_process():
        ...
        model_path = (
            os.path.join(ckpdir, f"{args.finetuning_mode}_zeroshot.pt")
            if linearized_finetuning
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in enumerate(ddp_loader):
            ...
            if (
                ...
                model_path = (
                    os.path.join(ckpdir, f"{args.finetuning_mode}_checkpoint_{step}.pt")
                    if linearized_finetuning
    ...
    if args.save is not None and is_main_process():
        ...
        ft_path = (
            os.path.join(ckpdir, f"{args.finetuning_mode}_finetuned.pt")
            if linearized_finetuning
