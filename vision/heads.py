...
def get_classification_head_INLT(args, dataset, split):
    if not dataset.endswith("Val"):
        # We want to load the head for the validation set always to be consistent with the one generated at training time.
        dataset += "Val"

    filename = os.path.join(args.save, 'INLT', f"head_{split}.pt")
    if os.path.exists(filename):
        print(f"Classification head for {args.model} on INLT[{split}] exists at {filename}")
        return ClassificationHead.load(filename)
    print(
        f"Did not find classification head for {args.model} on INLT[{split}] at {filename}, building one from scratch."  # noqa: E501
    )
    model = ImageEncoder(args, keep_lang=True).model
    template = get_templates(dataset)
    classification_head = build_classification_head(
